defmodule AppWeb.UserAuth do
  ...
  def on_mount(:require_onboarding, _params, session, socket) do
    ...
    if socket.assigns.current_scope &&
         App.Accounts.Scope.onboarding_complete?(socket.assigns.current_scope) do
      {:cont, socket}
    else
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:info, "Complete seu cadastro para continuar.")
  ...
  def signed_in_path(_), do: ~p"/condominios"
